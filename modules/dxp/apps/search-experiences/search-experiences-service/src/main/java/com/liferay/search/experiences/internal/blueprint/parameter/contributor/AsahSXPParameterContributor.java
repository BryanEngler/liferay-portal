...
public class AsahSXPParameterContributor implements SXPParameterContributor {
	...
	public void contribute(
		Map<String, Object> uiConfigurationValues, int mostTrackedContentSize,
		int userMostTrackedContentSize, SearchContext searchContext,
		...
		if (mostTrackedContentSize > 0) {
			_addSXPParameters(
				analyticsConfiguration, MOST_TRACKED_CONTENT_PARAMETER_PREFIX,
				searchContext,
				mostTrackedContentSize, sxpParameters, uiConfigurationValues, 0);
		}

		if (userMostTrackedContentSize > 0) {
			...
			_addSXPParameters(
				...
				USER_MOST_TRACKED_CONTENT_PARAMETER_PREFIX, searchContext,
				userMostTrackedContentSize, sxpParameters,
				uiConfigurationValues, userId);
		}
	}

	public static final String MOST_TRACKED_CONTENT_PARAMETER_PREFIX =
		"analytics.most_tracked_content_";

	public static final String USER_MOST_TRACKED_CONTENT_PARAMETER_PREFIX =
		"analytics.user_most_tracked_content_";
	...
	public List<SXPParameterContributorDefinition>
		...
		return Arrays.asList(
			new SXPParameterContributorDefinition(
				...
				MOST_TRACKED_CONTENT_PARAMETER_PREFIX + "X_uid"),
			new SXPParameterContributorDefinition(
				StringSXPParameter.class, "user-most-tracked-content-X-uid",
				USER_MOST_TRACKED_CONTENT_PARAMETER_PREFIX + "X_uid"));
	}
			...
			try {
				_populateSXPParameters(
					JSONFactoryUtil.createJSONArray(
						asahCacheEntry.getMostTrackedContents()),
	...
	private String _getDocumentId(
		...
		if (Validator.isNotNull(uiConfigurationValues.get("filter_value"))) {
			sb.append("-K");
	...
	private void _setAsahCacheEntryValues(
		...
		asahCacheEntry.setMostTrackedContents(jsonArray.toString());
