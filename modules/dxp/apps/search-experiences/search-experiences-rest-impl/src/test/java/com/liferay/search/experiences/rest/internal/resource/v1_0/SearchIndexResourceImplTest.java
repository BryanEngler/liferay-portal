	...
	public void testGetSearchIndexes() {
		...
		_setUpSearchEngineAdapter(
			new String[] {
				...
				"external:this-is-an-external-index",
				"this-should-be-filtered-out-index"
			});

		List<SearchIndex> searchIndexes =
			_searchIndexResourceImpl.getSearchIndexes();

		Assert.assertEquals(searchIndexes.toString(), 3, searchIndexes.size());
		...
		searchIndex = searchIndexes.get(2);

		Assert.assertEquals("this-is-an-external-index", searchIndex.getName());
	}
