...
public class DateRangeTermFilterTranslatorImpl
	...
	public Query translate(DateRangeTermFilter dateRangeTermFilter) {
		...
		if (!_dateFormatPattern.equals(dateFormat) ||
			...
			try {
				if (lowerBound != null) {
					lowerBound = ZonedDateTimeUtil.formatDate(
						dateFormats, _dateFormatPattern, lowerBound,
						timeZone.toZoneId(), _TIME_ZONE.toZoneId());
				}

				if (upperBound != null) {
					upperBound = ZonedDateTimeUtil.formatDate(
						dateFormats, _dateFormatPattern, upperBound,
						timeZone.toZoneId(), _TIME_ZONE.toZoneId());
				}
			}
			catch (Exception e) {
