...
public class SearchBarPortlet
	...
	protected Optional<Long> getThisSiteGroupId(
		...
		if (!thisSiteSearchScopeOptional.isPresent()) {
			searchScopeOptional = getSearchScope(
				searchBarPortletPreferences, portletSharedSearchSettings);

			thisSiteSearchScopeOptional = searchScopeOptional.filter(
				SearchScope.THIS_SITE::equals);
		}
